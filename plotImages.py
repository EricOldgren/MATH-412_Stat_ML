...
def get_dataloaders(batch_size, num_workers=0, train_transforms=None, test_transforms=None):
    ...
    test_loader = DataLoader(dataset=test_dataset, batch_size=4, num_workers=num_workers, shuffle=False)
...
class AutoEncoder(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
                ...
                #nn.LeakyReLU(0.01),
                #nn.MaxPool2d(kernel_size=2, stride=2),
                nn.Conv2d(128, 3, stride=1, kernel_size=3, padding=1),
        )
        self.decoder = nn.Sequential(
                nn.ConvTranspose2d(3, 128, stride=1, kernel_size=3, padding=1),
                #nn.LeakyReLU(0.01),
                #nn.Upsample(16),
...
model.load_state_dict(torch.load('16_16_3.pt', weights_only=True))
    ...
    for batch_idx, (test_features, _) in enumerate(test_loader):
        ...
        for idx, image in enumerate(test_features):
            if batch_idx == 0:
                ...
                plt.imsave(f'outputJPEG{idx}.png', enc)
                    
                ax[0, idx].imshow(input_image)
                ax[1, idx].imshow(enc)
                ax[2, idx].imshow(compr_image)
                ax[3, idx].imshow(output_image)

for axs in ax:
    for a in axs:
        a.set_xticks([])
        a.set_yticks([])
plt.savefig('reconstructedImages.png')
plt.show()
